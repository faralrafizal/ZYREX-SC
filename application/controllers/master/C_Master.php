...
class C_Master extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['count_admin']  = $this->M_Master->admin_count()->result();
		$data['data_pm']	= $this->M_Master->tb_pm()->result();
		$data['data_teknisi']	= $this->M_Master->tb_teknisi()->result();
		$data['count_user'] = $this->M_Master->user_count()->result();
		$this->load->view('master/Va_dashboard', $data);
	}
	...
	public function create_master()
	{
		...
		$this->load->view('master/Va_master-create', $data);
	}
	public function create_master_process()
	{
		...
		{  
			$this->load->view('master/Va_master-create');
		}
		else
		{
			...
			$myurl1 = 'master/C_Master/read_master';
					echo 
						'
						<script>
	...
	{
		$data['data_master'] = $this->M_Master->tb_master()->result();
        $this->load->view('master/Va_master-read', $data);
	}
	...
	public function update_master($master_id)
	{
        ...
        $this->load->view('master/Va_master-update', $data);
	}
	...
	public function create_admin()
	{
		...
		$this->load->view('master/Va_admin-create', $data);
	}
	public function create_admin_process()
	{
		...
		if($this->form_validation->run() == FALSE)
		{  
			$this->load->view('master/Va_admin-create');
		}
		else
		{
			...
			// redirect(site_url('admin/C_Admin/read_admin'));
			$myurl1 = 'master/C_Master/read_admin';
					echo 
						'
						<script>
							window.location.href = "'.base_url().'index.php/'.$myurl1.'";
	...
	{
		$data['data_admin'] = $this->M_Admin->tb_admin()->result();
        $this->load->view('master/Va_admin-read', $data);
	}
	...
	public function update_admin($admin_id)
	{
        ...
        $this->load->view('master/Va_admin-update', $data);
	}
	...
	public function create_pm()
	{
		//GET REQUIRED DATA FROM DB
        $data['data_master']		= $this->M_Master->tb_master()->result();
		$data['data_admin']		= $this->M_Master->tb_admin()->result();
		$data['data_pm']	= $this->M_Master->tb_pm()->result();
		$data['data_teknisi']	= $this->M_Master->tb_teknisi()->result();
		$data['data_user']	= $this->M_Master->tb_user()->result();

		$this->load->view('master/Va_pm-create', $data);
	}
	public function create_pm_process()
	{
		...
		if($this->form_validation->run() == FALSE)
		{  
			$this->load->view('master/Va_pm-create');
		}
		else
		{
	...
	{
		$data['data_pm'] = $this->M_Master->tb_pm()->result();
        $this->load->view('master/Va_pm-read', $data);
	}
	...
	{
		//GET REQUIRED DATA FROM DB
		$data['data_pm'] = $this->M_Master->pm_id($pm_id)->row();
        $this->load->view('master/Va_pm-update', $data);
	}
	public function update_pm_process()
	{
		...
		else
		{
        	...
        	$this->M_Master->update_pm($pm_id, $data);
			echo "<script>
	...
	{
        $this->M_Master->delete_pm($pm_id);
		// redirect(site_url('admin/C_Admin/read_pm'));
		$myurl2 = 'master/C_Master/read_pm';
					echo 
						'
						<script>
	...
	public function create_teknisi()
	{
		...
		$this->load->view('master/Va_teknisi-create', $data);
	}
	public function create_teknisi_process()
	{
		...
		if($this->form_validation->run() == FALSE)
		{  
			$this->load->view('master/Va_teknisi-create');
		}
		else
		{
			...
			$data['level']			= 3;
			$this->M_Master->create_teknisi($data);
			echo "<script>
	...
	{
		$data['data_teknisi'] = $this->M_Master->tb_teknisi()->result();
        $this->load->view('master/Va_teknisi-read', $data);
	}
	...
	{
		//GET REQUIRED DATA FROM DB
		$data['data_teknisi'] = $this->M_Master->teknisi_id($teknisi_id)->row();
        $this->load->view('master/Va_teknisi-update', $data);
	}
	public function update_teknisi_process()
	{
		...
		else
		{
        	...
        	$this->M_Master->update_teknisi($teknisi_id, $data);
			echo "<script>
	...
	{
        $this->M_Master->delete_teknisi($teknisi_id);
 
		$myurl3 = 'master/C_Master/read_teknisi';
					echo 
						'
						<script>
	...
	public function create_user()
	{
		...
		$this->load->view('master/Va_user-create', $data);
	}
	public function create_user_process()
	{
		...
		if($this->form_validation->run() == FALSE)
		{  
			$this->load->view('master/Va_user-create');
		}
		else
		{
	...
	{
		$data['data_user'] = $this->M_Master->tb_user()->result();
        $this->load->view('master/Va_user-read', $data);
	}
	...
	{
		//GET REQUIRED DATA FROM DB
		$data['data_user'] = $this->M_Master->user_id($user_id)->row();
        $this->load->view('master/Va_user-update', $data);
	}
	public function update_user_process()
	{
		...
		else
		{
        	...
        	$this->M_Master->update_user($user_id, $data);
	...
	{
        $this->M_Master->delete_user($user_id);
		// redirect(site_url('admin/C_Admin/read_user'));  
		$myurl4 = 'master/C_Master/read_master';
					echo 
						'
						<script>
	...
	{
		$data['data_komplain'] = $this->M_Master->tb_komplain()->result();
        $this->load->view('master/Va_komplain-read', $data);
	}
	
    //DELETE
	public function delete_komplain($komplain_id)
	{
        $this->M_Master->delete_komplain($komplain_id);
		// redirect(site_url('admin/C_Admin/read_komplain'));  
		$myurl5 = 'master/C_Master/read_komplain';
					echo 
						'
						<script>
	...
	{
		$data['data_servis'] = $this->M_Master->tb_servis()->result();
        $this->load->view('master/Va_servis-read', $data);
	}
	
    //DELETE
	public function delete_servis($nota)
	{
        $this->M_Master->delete_servis($nota);
		// redirect(site_url('admin/C_Admin/read_servis')); 
		$myurl6 = 'master/C_Master/read_servis';
					echo 
						'
						<script>
	...
	{
		//GET REQUIRED DATA FROM DB
		$data['data_servis'] = $this->M_Master->nota($nota)->row();
        $this->load->view('master/Va_servis-zoom', $data);
	}
	...
	function periode(){
        ...
        $this->load->view('master/Va_laporan-read');
    }
	...
	{
		//GET REQUIRED DATA FROM DB
		$data['data_perusahaan'] = $this->M_Master->perusahaan_id($perusahaan_id)->row();
        $this->load->view('master/Va_perusahaan', $data);
	}
    ...
    public function update_perusahaan()
	{
		...
		if($this->form_validation->run() == FALSE)
		{  
			$this->load->view('master/Va_perusahaan');
		}
		else
		{
        	...
        	$this->M_Master->update_perusahaan($perusahaan_id, $data);
	...
	{
		$data['read_customer'] = $this->M_Master->tb_customer()->result();
        $this->load->view('master/Va_customer-read', $data);
	}

	//REDIRECT
	public function create_customer()
	{
		$data['data_customer']		= $this->M_Master->tb_customer()->result();
		$this->load->view('master/Va_customer-create', $data);
	}
	...
	public function create_customer_process()
	{
		...
		if($this->form_validation->run() == FALSE)
		{  
			$this->load->view('master/Va_customer-create');
		}
		else
		{
			...
			$this->M_Master->create_customer($data);
			echo "<script>
	...
	{
		//GET REQUIRED DATA FROM DB
		$data['data_customer'] = $this->M_Master->customer_id($customer_id)->row();
        $this->load->view('master/Va_customer-update', $data);
	}
	...
	public function update_customer_process()
	{
        	...
        	$this->M_Master->update_customer($customer_id, $data);
			echo "<script>
	...
	public function zoom_customer($customer_id)
	{
		//GET REQUIRED DATA FROM DB
		$data['data_customer'] = $this->M_Master->customer_id($customer_id)->row();
        $this->load->view('master/Va_customer-zoom', $data);
	}

	//DELETE
	public function delete_customer($customer_id)
	{
        $this->M_Master->delete_customer($customer_id);
 
		$myurl3 = 'master/C_Master/read_customer';
					echo 
						'
						<script>
							window.location.href = "'.base_url().'index.php/'.$myurl3.'";
