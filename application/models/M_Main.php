...
{
	public function get_master($username,$password)
	{
		$query = $this->db->query("SELECT * FROM master WHERE master_user='$username' AND master_pass='$password' ");
		return $query;
	}
	...
	{
		$query = $this->db->query("SELECT * FROM dataservis WHERE tlpn='$username' AND nota='$password' ");
		return $query;
	}
}
